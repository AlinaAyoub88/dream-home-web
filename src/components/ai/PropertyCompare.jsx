 ...
 * Follows the Dream Home design standards (Deep Navy #1A2A3A & Royal Gold #C9A84C).
 */
const PropertyCompare = () => {
  // Mock data representing properties selected for comparison matching Firestore schema
        ...
        <div className="bg-white rounded-2xl shadow-xl border border-[#F5F0EB] overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full min-w-[600px] border-collapse text-left">
              ...
              {/* TABLE HEADER: Image/Title Blocks */}
              <thead>
                <tr className="bg-[#1A2A3A] text-white">
                  ...
                  {comparedProperties.map((item) => (
                    <th key={item.propertyId} className="p-4 text-sm font-semibold w-1/3 border-b border-[#C9A84C] relative">
                      <div className="pr-6">
              ...
              <tbody className="divide-y divide-gray-100 text-sm text-gray-700">
                ...
                <tr className="hover:bg-gray-50/70 transition-colors">
                  ...
                  {comparedProperties.map((item) => (
                    <td key={item.propertyId} className="p-4 font-bold text-[#1A2A3A] text-base">
                ...
                <tr className="hover:bg-gray-50/70 transition-colors">
                  ...
                  {comparedProperties.map((item) => (
                    <td key={item.propertyId} className="p-4 font-medium">
                ...
                <tr className="hover:bg-gray-50/70 transition-colors">
                  ...
                  {comparedProperties.map((item) => (
                    <td key={item.propertyId} className="p-4 text-xs text-gray-600">
                      {item.location}
                    </td>
                  ))}
                </tr>
                    ...
                    <td key={item.propertyId} className="p-4">
                      <div className="flex flex-wrap gap-1.5">
                        {item.features.map((feat, idx) => (
                          <span key={idx} className="bg-[#F5F0EB] text-[#1A2A3A] text-xs px-2.5 py-1 rounded-md font-medium border border-gray-200/60">
                            {feat}
                          </span>
                        ))}
                      </div>
                ...
                <tr className="hover:bg-gray-50/70 transition-colors">
                  ...
                  {comparedProperties.map((item) => (
                    <td key={item.propertyId} className="p-4 text-xs font-semibold text-emerald-600 flex items-center gap-1">
